...
export default class ScheduleBuilder {
  constructor(state) {
    this.state = state;
    this.printOnly = !!state.printOnly;
    this.tableBuilder = new TableBuilder({ class: "table table-sm table-bordered schedule-grid" });
  ...
  buildTimesColumn() {
    ...
    for (let i = 0; i < times.length-1; i++) {
        ...
        if (i > 0 && startTime % 60 === 0) {
          this.tableBuilder.hRule(startRow);
        }
    }
  }
  ...
  buildDayHeaders() {
    for (let iDay of this.iDays) {
        ...
        this.tableBuilder.vRule(col);
    }
  }


  buildItem(item) {
    // item = { name, loc, days, time: [start, end], links?: { syllabus, assignments, repo } }
    ...
    const linkSpecs = this.printOnly ? [] : [
      { key: "syllabus", label: "Syllabus" },
      { key: "assignments", label: "Assignments" },
      { key: "repo", label: "Repo" },
    ].filter(({ key }) => item.links && item.links[key]);
    ...
    for (let d of item.days) {
      ...
      const text  = (
        <div>
          ...
          {linkSpecs.length > 0 && (
            <div>
              {linkSpecs.map(({ key, label }, i) => (
                <React.Fragment key={key}>
                  {i > 0 && " | "}
                  <a href={item.links[key]} target="_blank" rel="noreferrer" style={{ textDecoration: "underline" }}>{label}</a>
                </React.Fragment>
              ))}
            </div>
          )}
  ...
  render() {
    ...
    const wrapperClassName = this.printOnly ? "schedule-main" : "col-sm-8 schedule-main print-full";
