...
export default class ScheduleBuilder {
  constructor(state) {
    this.state = state;
    this.table = new Tabular({ class: "table table-sm table-bordered" });
  ...
  buildTimesColumn() {
    ...
    const timeCol = 0;
    this.table.header(0, 0, timeCol, timeCol, {}, "Time");
    ...
    for (let i = 0; i < times.length-1; i++) {
        ...
        this.table.addCell(startRow, endRow, timeCol, timeCol, {}, text);
    }
  }
  ...
  buildDayHeaders() {
    for (let iDay of this.iDays) {
        ...
        this.table.header(0,0,col,col,{},text);
    }
  }
  ...
  buildItem(item) {
    ...
    const endRow   = this.timeRow(iEnd);
    ...
    for (let d of item.days) {
      ...
      this.table.addCell(startRow, endRow, col, col, attrs, text);
    }
  }
  ...
  render() {
    ...
    const table = this.table.generate();
